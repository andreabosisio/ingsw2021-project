...
public class FaithTrackTest {
    ...
    void testOnePlayerTile1False() {
        ...
        assertEquals(0, GameBoard.getGameBoard().getFaithTrackPlayer(players.get(0)).getVictoryPoints());
    }
    ...
    void testOnePlayerTile1True() {
        ...
        assertTrue(GameBoard.getGameBoard().getFaithTrackPlayer(players.get(0)).isPopeTile1()); // The first tile
        assertEquals(2, GameBoard.getGameBoard().getFaithTracks().size());
        assertEquals(2 + 2, GameBoard.getGameBoard().getFaithTrackPlayer(players.get(0)).getVictoryPoints());
    }
    ...
    void testOnePlayerWithAllThePopeTileFlippedUp() {
        ...
        assertTrue(GameBoard.getGameBoard().getFaithTrackPlayer(players.get(0)).isPopeTile1()); // The first tile
        assertTrue(GameBoard.getGameBoard().getFaithTrackPlayer(players.get(0)).isPopeTile2()); // The secondo tile
        assertTrue(GameBoard.getGameBoard().getFaithTrackPlayer(players.get(0)).isPopeTile3()); // The third tile
        // Roronoa's progress: 9 + 7 + 9 = 25 --> Without controls, his Faith Marker goes out of the Faith Track
        assertEquals(endOfTheFaithTrack, GameBoard.getGameBoard().getFaithTrackPlayer(players.get(0)).getFaithMarker());
        assertEquals(2, GameBoard.getGameBoard().getFaithTracks().size());
        assertEquals(20 + 2 + 3 + 4, GameBoard.getGameBoard().getFaithTrackPlayer(players.get(0)).getVictoryPoints());
    }
    ...
    void testTwoTileTrueOfFourPlayers() {
        ...
        assertTrue(GameBoard.getGameBoard().getFaithTrackPlayer(players.get(0)).isPopeTile1()); // The Roronoa's tile is flipped up
        assertTrue(GameBoard.getGameBoard().getFaithTrackPlayer(players.get(1)).isPopeTile1()); // The Monkey's tile is flipped up
        assertFalse(GameBoard.getGameBoard().getFaithTrackPlayer(players.get(2)).isPopeTile1()); // The Nico's tile is not flipped up
        assertFalse(GameBoard.getGameBoard().getFaithTrackPlayer(players.get(3)).isPopeTile1()); // The Vinsmoke's tile is not flipped up

        assertEquals(4, GameBoard.getGameBoard().getFaithTracks().size());

        assertEquals(2 + 2, GameBoard.getGameBoard().getFaithTrackPlayer(players.get(0)).getVictoryPoints());
        assertEquals(1 + 2, GameBoard.getGameBoard().getFaithTrackPlayer(players.get(1)).getVictoryPoints());
        assertEquals(1, GameBoard.getGameBoard().getFaithTrackPlayer(players.get(2)).getVictoryPoints());
        assertEquals(0, GameBoard.getGameBoard().getFaithTrackPlayer(players.get(3)).getVictoryPoints());
    }
    ...
    void testOneTimeFlippedUpTheSameTile() {
        ...
        assertEquals(2 + 2, GameBoard.getGameBoard().getFaithTrackPlayer(players.get(0)).getVictoryPoints());
        assertEquals(2, GameBoard.getGameBoard().getFaithTrackPlayer(players.get(1)).getVictoryPoints());
    }
    ...
    void testAllPlayersExceptOneProgress() {
        ...
        assertFalse(GameBoard.getGameBoard().getFaithTrackPlayer(players.get(1)).isPopeTile1()); // The Monkey's tile is not flipped up
        assertTrue(GameBoard.getGameBoard().getFaithTrackPlayer(players.get(2)).isPopeTile1()); // The Nico's tile is flipped up
        assertTrue(GameBoard.getGameBoard().getFaithTrackPlayer(players.get(3)).isPopeTile1()); // The Vinsmoke's tile is flipped up

        assertEquals(4, GameBoard.getGameBoard().getFaithTracks().size());

        assertEquals(1, GameBoard.getGameBoard().getFaithTrackPlayer(players.get(0)).getVictoryPoints());
        assertEquals(1, GameBoard.getGameBoard().getFaithTrackPlayer(players.get(1)).getVictoryPoints());
        assertEquals(1 + 2, GameBoard.getGameBoard().getFaithTrackPlayer(players.get(2)).getVictoryPoints());
        assertEquals(2 + 2, GameBoard.getGameBoard().getFaithTrackPlayer(players.get(3)).getVictoryPoints());
    }
}