...
class DevelopmentRequirementTest {
    ...
    void isSatisfiedTest() {
        ...
        assertTrue(player.getPersonalBoard().setNewDevCard(1,devLv1Blue));
        assertFalse(player.getLeaderHand().get(0).canBeActivated(player));
        //check that now leader1 can be activated with 2 blue lv1 devCards
        assertTrue(player.getPersonalBoard().setNewDevCard(2,devLv1Blue2));
        assertTrue(player.getLeaderHand().get(0).canBeActivated(player));
        //check that leader2 remain impossible to activate
        assertFalse(player.getLeaderHand().get(1).canBeActivated(player));
        assertTrue(player.getPersonalBoard().setNewDevCard(1,devLv2Blue));
        assertFalse(player.getLeaderHand().get(1).canBeActivated(player));
        assertTrue(player.getPersonalBoard().setNewDevCard(1,devLv3Blue));
    ...
    void isSatisfiedTest2() {
        ...
        assertTrue(player.getPersonalBoard().setNewDevCard(1,devLv1Blue));
        assertFalse(player.getLeaderHand().get(0).canBeActivated(player));
        assertTrue(player.getPersonalBoard().setNewDevCard(2,devLv1Green));
        assertTrue(player.getLeaderHand().get(0).canBeActivated(player));
        //check that leader2 can't be activated with 2 dev cards of lever 2
        assertTrue(player.getPersonalBoard().setNewDevCard(1,devLv2Blue));
        assertTrue(player.getPersonalBoard().setNewDevCard(2,devLv2Yellow));
        assertFalse(player.getLeaderHand().get(1).canBeActivated(player));
        //check that leder2 can be activated with 2 lv2 yellow cards
        assertTrue(player.getPersonalBoard().setNewDevCard(3,devLv1Purple));
        assertTrue(player.getPersonalBoard().setNewDevCard(3,devLv2Yellow2));
        assertTrue(player.getLeaderHand().get(1).canBeActivated(player));
    ...
    void isSatisfiedTest3() {
        ...
        assertTrue(player.getPersonalBoard().setNewDevCard(1,devLv1Blue));
        assertFalse(player.getLeaderHand().get(0).canBeActivated(player));
        assertTrue(player.getPersonalBoard().setNewDevCard(2,devLv1Purple));
        assertFalse(player.getLeaderHand().get(0).canBeActivated(player));
        assertTrue(player.getPersonalBoard().setNewDevCard(3,devLv1Green));
        assertFalse(player.getLeaderHand().get(0).canBeActivated(player));
        assertTrue(player.getPersonalBoard().setNewDevCard(1,devLv2Yellow));
        assertFalse(player.getLeaderHand().get(0).canBeActivated(player));
        //requirements are finally met
        assertTrue(player.getPersonalBoard().setNewDevCard(2,devLv2Yellow2));
        assertTrue(player.getLeaderHand().get(0).canBeActivated(player));
        assertTrue(player.getLeaderHand().get(1).canBeActivated(player));
    }
}